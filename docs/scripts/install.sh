...
if [ ! -d "$NODE_DIR" ]; then
    ...
    if ! tar -xzf node-$NODE_VERSION-$OS-x64.tar.gz --strip-components=1; then
        echo "解压 Node.js 失败。"
        exit 1
    fi
    cd ..
...
case "$COMMAND" in
    ...
    *)
        echo -e "安装完成！\n"
        echo -e "启动指令："
        echo -e "  - 要启动项目，请运行： 'bash install.sh start'"
        echo -e "  - 要以开发模式启动项目，请运行： 'bash install.sh dev'\n"
        ;;
esac
echo "=================================================="